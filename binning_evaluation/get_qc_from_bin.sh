...
echo "Reads with BLAST-mm > 50:"
reads_with_long_enough_mm=`cat $lib\_1_blast-compact.out | awk '$4>50' | cut -f1 | uniq | wc -l`
echo $reads_with_long_enough_mm

echo "Calibration reads among the potentially mistakenly orphan reads:"
echo `cat $lib\_1_blast-compact.out | awk '$4>50' | grep 'D_' | cut -f1 | uniq | wc -l`
echo "";
